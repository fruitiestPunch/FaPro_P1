...
def reduce_list(data):
    ...
    data_list = []
    for key in sum_e:
        data_list.append({'name': key, 'objective': sum_o[key] / count[key], 'execution_time': sum_e[key] / count[key],
                          'process_time': sum_p[key] / count[key]})
...
def beautify_algo_name(plot_data):
    for algo in plot_data:
        ...
        algo['name'] = algo['name'].replace("heur_ospf_weights", "OSPF")
        algo['name'] = algo['name'].replace("uniform_weights", "UW")
        algo['name'] = algo['name'].replace("demand_first_waypoints", "DFW")
        algo['name'] = algo['name'].replace("sequential_combination", "JointHeur")
        algo['name'] = algo['name'].replace("sequential_combination_2", "JointHeur_2")
...
if __name__ == "__main__":
    ...
    for filename in json_filenames:
        ...
        json_data = list(map(
            lambda json_dict: {'name': json_dict['algorithm'], 'objective': json_dict['objective'],
