...
Uses the inverse capacity and the betweeness centrality metric to set weights along the links.
"""
class BetweenessInverseCapacity(GenericSR):
    ...
    def solve(self) -> dict:
        """ set weights to inverse capacity divided by weighted betweeness centrality and use shortest path algorithm """
        ...
        self.__calculated_betweennesscentrality()
    ...
    def __calculated_betweennesscentrality(self):
        """" sets weights to the current weight value divide by the average of the normalizied betweenness-centralities"""
        betweenness = nk.centrality.Betweenness(self.__g, normalized = True)
        betweenness.run()
        centralities = betweenness.ranking()
    ...
    def get_name(self):
        ...
        return f"betweeness_inverse_capacity"
