...
Uses the inverse capacity and the closeness centrality metric to set weights along the links.
"""
class ClosenessInverseCapacity(GenericSR):
    ...
    def solve(self) -> dict:
        """ set weights to inverse capacity divided by weighted closeness centrality and use shortest path algorithm """
        ...
        self.__calculated_closenesscentrality()
    ...
    def __calculated_closenesscentrality(self):
        """" sets weights to the current weight value divide by the average of the normalizied closeness-centralities"""
        closeness = nk.centrality.Closeness(self.__g, True, False)
        closeness.run()
        centralities = closeness.ranking()
    ...
    def get_name(self):
        ...
        return f"closeness_inverse_capacity"
